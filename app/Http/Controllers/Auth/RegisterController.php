...
class RegisterController extends Controller
{
    ...
    */
    ...
    protected function ownerValidator(array $data)
    {
        return Validator::make($data, [
            'name' => ['required', 'string', 'max:255'],
    ...
    {
        return view('auth.register', ['authgroup' => 'owner']);
    }

    public function registerOwner(Request $request)
    {
        $this->ownerValidator($request->all())->validate();

        event(new Registered($user = $this->createOwner($request->all())));

        Auth::guard('owner')->login($user);

        if ($response = $this->registeredOwner($request, $user)) {
            return $response;
        }
    ...
    protected function registeredOwner(Request $request, $user)
    {
        //
    }
